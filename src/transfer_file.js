(params) => {
  ...
  if (source && target) {
    var source_blob = api.run("this.find_blob_object", {
      owner: source.owner,
      path: source.path,
      repo: source.repo,
      branch: source.branch,
    })[0];
    var target_blob = api.run("this.find_blob_object", {
      owner: target.owner,
      path: target.path,
      repo: target.repo,
    ...
    var body = {
      committer: { name: github_user.login, email: commitEmail },
    ...
    try {
      api.run("github.add_file_to_repo", {
        owner: target.owner,
        path: target.path,
        repo: target.repo,
        $body: body,
  ...
  function parse_github_url(url) {
    ...
    var url_match =
      /(https:\/\/)*(github.com\/)*(.+?)\/(.+?)\/blob\/(.+?)\/(.+)/.exec(url);
    if (url_match) {
      return {
        owner: url_match[3],
        repo: url_match[4],
